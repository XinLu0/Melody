    ...
    <div class="row">
      ...
      for ($i = 1; $i <= $sum; $i ++){
        ...
        $prop = getMajorPropsSGByItemNo($i);
          ...
          for($i = 1; $i <= $total_item; $i ++)
          {
            $subNum = getNumOfItemByItemIDAndMemeberID($subMemId,$i);
            $price = getPriceCNByItemNo($i);
            if(getIsInChinaByMemberId($userId) == 0)
            {
              $price = getPriceSGByItemNo($i);
            ...
            $subincome = $subincome+$price*$subNum*$prop;
                  echo "<td id=\"Minor1\">$subNum</td>";
          }
            ...
            echo "<th id=\"Performance\">Credit Change</th>";
            echo "<th id=\"Performance\"> </th>";
            for($i = 1; $i <= $total_item; $i ++){
              echo "<th id=\"Performance\"> </th>";
            }
            echo "<th id=\"Performance\">$creditChange</th>";
            echo "</tr>";
            //Last Total
            $totalIncome += $creditChange;
            echo "<tr id=\"Performance\">";
